...
function markingStar(e) {
    ...
    while (idx <= click_star) {
        if (idx === click_star) {
            if (e.offsetX <= e.target.width / 2) {
                document.querySelector(`#star${idx}`).src = "/static/image/half.png";
            } else {
                document.querySelector(`#star${idx}`).src = "/static/image/full.png";
            }
        } else {
            document.querySelector(`#star${idx}`).src = "/static/image/full.png";
        }
    ...
    while (remain_idx <= 5) {
        document.querySelector(`#star${remain_idx}`).src = "/static/image/empty.png";
        remain_idx++;
    }
    checkScore();
}
...
function dragingStar(e) {
    ...
    while (idx <= drag_star) {
        if (idx === drag_star) {
            if (half) {
                document.querySelector(`#star${idx}`).src = "/static/image/full.png";
                document.querySelector(`#star${idx + 1}`).src = "/static/image/half.png";
            } else {
                document.querySelector(`#star${idx}`).src = "/static/image/full.png";
            }
        } else {
